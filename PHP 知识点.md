...
// strstr、stristr、strpos这三个函数的区别  
    ...
    strpos的性能比较好，如果只是判断needle是否在字符串haystack中，则使用strpos较好
    它将占用更少的内存和获得更快的执行速度。  
    ...
    strpos      区别大小写strpos查找成功后则是返回的是位置。
                因为位置有可能是0，所以判断查找失败使用===false更合适。  
...
//在HTTP/1.0中，状态码401的含义是(?);如果返回"找不到文件"的提示，则可用 header 函数，其语句为(?)  
    ...
    304 Not Modified        客户端发送了一个带条件的GET请求且已被允许
...
// Mysql常用引擎的特点  
    ...
    MERGE表本身没有数据，对MERGE类型的表进行查询、更新、删除的操作，就是对内部的MyISAM表进行的。  
