...
```
Docker镜像（Image）类似于虚拟机的镜像
可以将他理解为一个面向Docker引擎的只读模板，包含了文件系统。

例如：一个镜像可以完全包含了Ubuntu操作系统环境，可以把它称作一个Ubuntu镜像。
镜像也可以安装了Apache应用程序（或其他软件），可以把它称为一个Apache镜像。

镜像是创建Docker容器的基础，通过版本管理和增量的文件系统
...
例如存放Ubuntu操作系统镜像的仓库，称为 Ubuntu 仓库，其中可能包括 14.04, 12.04 等不同版本的镜像。
...
当用户创建了自己的镜像之后就可以使用push将它上传到指定的公有或则私有仓库。
...
MAINTAINER		用来指定镜像创建者信息
				...
				构建指令，用于将image的制作者相关的信息写入到 image 中
...
CMD				设置 container 启动时执行的操作
				...
				该指定的的脚本或者程序将会以param1和param2作为参数执行
...
ENTRYPOINT		设置 container 启动时执行的操作
				...
				当独自使用时，如果你还使用了CMD命令且CMD是一个完整的可执行的命令
...
EXPOSE			指定容器需要映射到宿主机器的端口
				...
				然后在运行容器的时候指定-p选项加上 EXPOSE 设置的端口
				这样 EXPOSE 设置的端口号会被随机映射成宿主机器中的一个端口号
				
				也可以指定需要映射到宿主机器的那个端口，这时要确保宿主机器上的端口号没有被使用
				EXPOSE 指令可以一次设置多个端口号，相应的运行容器的时候， 可以配套的多次使用-p选项
...
ENV				用于设置环境变量
				...
				# 假如你安装了JAVA程序，需要设置JAVA_HOME，那么可以在 Dockerfile 中这样写
